...
def erb(neta, nedan):
  os.makedirs('./attach/sushi_resize/' + neta + '/' + nedan, exist_ok=True)
  image_list = glob.glob('./15_repeat_erb_sushi_r/' + neta + '/' + nedan + '/*')
  print(image_list)
...
def resize_image(pic):
    #調整後サイズを指定(横幅、縦高さ)
    size=(64,64)
    output = pic.replace('15_repeat_erb_sushi_r', 'attach/sushi_resize')
