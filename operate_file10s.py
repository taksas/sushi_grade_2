...
def erb(neta, nedan):
  os.makedirs('./attach/sushi_rotate1_s/' + neta + '/' + nedan, exist_ok=True)
  os.makedirs('./attach/sushi_rotate2_s/' + neta + '/' + nedan, exist_ok=True)
...
def rotate_image1(pic):
    output = pic.replace('sushi_resize_s', 'sushi_rotate1_s')
    image = cv2.imread(pic)
    for ang in [-5, 0, 5]:
...
def rotate_image2(pic):
    output = pic.replace('sushi_resize_s', 'sushi_rotate2_s')
    image = cv2.imread(pic)
    for ang in [5, 0, -5]:
        image_rot = ndimage.rotate(image, ang)
        cv2.imwrite(output, image_rot)
