...
def erb(neta, nedan):
  os.makedirs('./attach/sushi_threshold_s/' + neta + '/' + nedan, exist_ok=True)
  os.makedirs('./attach/sushi_gaussian_s/' + neta + '/' + nedan, exist_ok=True)
  os.makedirs('./attach/sushi_mosaic_s/' + neta + '/' + nedan, exist_ok=True)
  image_list = glob.glob('./attach/sushi_resize_s/' + neta + '/' + nedan + '/*')
  print(image_list)
...
def threshold_image(pic):
    output = pic.replace('sushi_resize_s', 'sushi_threshold_s')
...
def gaussian_image(pic):
    output = pic.replace('sushi_resize_s', 'sushi_gaussian_s')
...
def mosaic_image(pic):
    output = pic.replace('sushi_resize_s', 'sushi_mosaic_s')
